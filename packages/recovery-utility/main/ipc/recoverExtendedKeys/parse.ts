...
type ExtendedKeys<P extends boolean> = P extends true
  ? FullExtendedKeys
  : ExtendedPublicKeys;
...
export const parseExtendedKeys = <P extends boolean>(
  stdout: string,
  dangerouslyRecoverPrivateKeys?: P
) => {
  ...
  if (
    dangerouslyRecoverPrivateKeys &&
    (!checkKey("xprv", xprv) || !checkKey("fprv", fprv))
  ) {
