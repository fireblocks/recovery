...
export const resolveIPCResponse = async <TRouter extends AnyRouter>({
  router,
  createContext,
  operation,
}: {
  ...
  const getResultEnvelope = (rawResult: RawResult): TRouterResponse => {
  ...
  const getEndResponse = (envelope: TRouterResponse): IPCResponse => {
