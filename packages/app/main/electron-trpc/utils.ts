...
export const getTRPCErrorFromUnknown = (cause: unknown): TRPCError => {
...
// from @trpc/server/src/error/utils
const getErrorFromUnknown = (cause: unknown): Error => {
...
export const transformTRPCResponseItem = <
...
>(
  ...
  if ("error" in item) {
    return {
      ...item,
      error: router._def._config.transformer.output.serialize(
        item.error
      ) as unknown,
    };
  }
