...
export const registerFileProtocol = (options: Options) => {
  ...
  _options.directory = path.resolve(
  ...
  const handler: ProtocolHandler = async (request, callback) => {
    ...
    const paths = [
      filePath,
      `${filePath}.html`,
      path.join(_options.directory, "404.html"),
    ];
  ...
  electron.app.on("ready", () => {
    const session = _options.partition
      ? electron.session.fromPartition(_options.partition)
  ...
  const loadURL = async (window_: electron.BrowserWindow, params?: string) => {
    await window_.loadURL(
      `${_options.scheme}://index.html${params ? `?${params}` : ""}`
    );
  };
