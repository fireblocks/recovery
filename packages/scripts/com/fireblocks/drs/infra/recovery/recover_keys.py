...
class RecoveryErrorPublicKeyNoMatch(RecoveryException):
    def __str__(self):
        return "Computed public key does not match expected public key (zip inconsistency)"
...
class RecoveryErrorKeyIdNotInMetadata(RecoveryException):
    ...
    def __str__(self):
        return "Found key id %s in zip file, but it doesn't exist in metadata.json" % self._key_id
...
class RecoveryErrorKeyIdMissing(RecoveryException):
    ...
    def __str__(self):
        return "metadata.json contains key id %s, which wasn't found in zip file" % self._key_id
...
def recover(zip_file: str,
            zip_prv_key: str,
            mobile_passphrase: str,
            zip_prv_key_pass: str = None):
    ECDSA = "MPC_ECDSA_SECP256K1"
    EDDSA = "MPC_EDDSA_ED25519"
    recovered_keys = restore_key_and_chaincode(zip_file, zip_prv_key, mobile_passphrase, zip_prv_key_pass)
...
def restore_key_and_chaincode(zip_file_b64: str,
                              zip_prv_key_b64: str,
                              mobile_passphrase: str,
                              zip_prv_key_pass: str = None):
    privkeys = {}
        ...
        for name in z.namelist():
            with z.open(name) as file:
                if name.startswith("MOBILE"):
                    ...
                    try:
                        data = decrypt_mobile_private_key(mobile_passphrase.encode(), obj["userId"].encode(),
                    ...
                    if len(data) == 36:  # the first 4 bytes encode the algorithm, and the rest is the private share
                        ...
                        data = data[4:]
                    players_data[key_id][get_player_id(key_id, obj["deviceId"], False)] \
                    ...
                    if key_id:
                        ...
                        players_data[key_id][get_player_id(key_id, cosigner_id, True)] = int.from_bytes(data,
                                                                                                        byteorder='big')
...
def calculate_keys(key_id, player_to_data, algo):
    if algo == "MPC_ECDSA_SECP256K1":
        ...
        for key, value in player_to_data.items():
            prv_key = (prv_key + value * lagrange_coefficient(key, player_to_data.keys(), secp256k1.q)) % secp256k1.q
        ...
        prv_key = 0
        for key, value in player_to_data.items():
            prv_key = (prv_key + value * lagrange_coefficient(key, player_to_data.keys(), ed25519.l)) % ed25519.l
