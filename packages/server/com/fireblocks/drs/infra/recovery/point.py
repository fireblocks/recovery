...
class Point:
    ...
    def __init__(self, x, y, curve=secp256k1):
        ...
        Args:
            ...
            |  curve (:class:`Curve`): The curve that the point lies on.
        """
        if not (x == 0 and y == 1 and curve is None) and not curve.is_point_on_curve(
            (x, y)
        ):
            raise ValueError(
    ...
    def __str__(self):
        if self == Point.IDENTITY_ELEMENT:
    ...
    def __add__(self, other):
        ...
        if self == Point.IDENTITY_ELEMENT:
            return other
        elif other == Point.IDENTITY_ELEMENT:
            return self
    ...
    def deserialize(pointStr, curve=secp256k1):
        ...
        if (pointStr.startswith("02") or pointStr.startswith("03")) and len(
            pointStr
        ) == 66:
