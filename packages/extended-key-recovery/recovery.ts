  ...
  if ('mobileRsaPath' in params || 'mobileRsaBase64' in params) {
    ...
    if (!decFunc) {
      ...
      const { stdout, stderr } = await execAsync(
        `python ./decrypt.py 2 ${rsaFileData} ${encryptedPassphrase} ${Buffer.from(params.rsaPass).toString('base64')}`,
      );
      if (stderr !== '') {
    ...
    if (file.entryName.startsWith('MOBILE')) {
      ...
      if (keyId) {
        ...
        if (!decFunc) {
          ...
          const execAsync = promisify(exec);
          const { stdout, stderr } = await execAsync(
            `python ./decrypt.py 1 ${Buffer.from(rsaFileData).toString('base64')} ${data} ${Buffer.from(params.rsaPass).toString(
              'base64',
            )}`,
          );
