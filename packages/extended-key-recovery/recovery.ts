...
async function recoverAutoGeneratedPassphrase(
  params: KeyRecoveryConfig,
  agpFile: IZipEntry,
  decFunc: Function | undefined,
): Promise<string> {
  if ('mobileRsaPath' in params || 'mobileRsaBase64' in params) {
    ...
    if (!decFunc) {
      ...
      const { stdout, stderr } = await execAsync(
        `python ./dec.py 2 ${rsaFileData} ${encryptedPassphrase} ${Buffer.from(params.rsaPass).toString('base64')}`,
      );
      if (stderr !== '') {
        throw new Error(`Sub execution failed`);
      }
      decrypted = Buffer.from(stdout.replace('0x', ''), 'hex').toString();
    } else {
      decrypted = await decFunc('2', rsaFileData, encryptedPassphrase, Buffer.from(params.rsaPass).toString('base64'));
      decrypted = Buffer.from(decrypted.replace('0x', ''), 'hex').toString();
    }
    return decrypted;
  }
...
export async function recoverKeys(params: KeyRecoveryConfig, decFunc: Function | undefined): Promise<RecoveredKeys> {
  ...
  const rsaKey: forge.pki.rsa.PrivateKey = forge.pki.decryptRsaPrivateKey(rsaFileData, params.rsaPass);
  ...
  const mobilePass =
    'mobilePass' in params ? params.mobilePass! : await recoverAutoGeneratedPassphrase(params, agpFile!, decFunc);
  ...
  for (const file of zipFiles) {
    if (file.entryName.startsWith('MOBILE')) {
      ...
      if (keyId) {
        const data = file.getData().toString('base64');
        let decrypted;
        if (!decFunc) {
          const { promisify } = require('util');
          const execAsync = promisify(exec);
          const { stdout, stderr } = await execAsync(
            `python ./dec.py 1 ${Buffer.from(rsaFileData).toString('base64')} ${data} ${Buffer.from(params.rsaPass).toString(
              'base64',
            )}`,
          );
          if (stderr !== '') throw new Error(`Sub execution failed`);

          decrypted = stdout;
        } else {
          decrypted = await decFunc(
            '1',
            Buffer.from(rsaFileData).toString('base64'),
            data,
            Buffer.from(params.rsaPass).toString('base64'),
          );
        }
        ...
        playerData[keyId][playerId] = BigInt(decrypted);
      }
    }
  }
  ...
  for (const keyId of Object.keys(playerData)) {
    ...
    if (pubFromMetadata !== pubKey) {
      ...
      console.error(
        `Failed to recover ${algo} key. Expected public key is ${pubFromMetadata} got: ${BigInt(pubKey).toString(16)}.`,
      );
      prvKey = '';
    } else {
      // @ts-ignore
