...
from com.fireblocks.drs.recovery import ed25519
from com.fireblocks.drs.recovery.curve import secp256k1
from com.fireblocks.drs.recovery.recovery_helper import encode_base58_checksum
...
class RecoveryErrorMPFRSADecrypt(RecoveryException):
    def __str__(self):
        return "Error decrypting auto generated mobile passphrase"
            ...
            with z.open(name) as file:
                if name.startswith("MOBILE"):
                    ...
                    try:
                        ...
                        # print("mobileKey: " + binascii.hexlify(data).decode())
                    except ValueError:
                    ...
                    if key_id:
                        r_file = file.read()
                        # print("Raw key data: " + binascii.hexlify(r_file).decode())
                        data = cipher.decrypt(r_file)
                        # print("Decrypted: " + binascii.hexlify(data).decode())
                        players_data[key_id][
...
def get_player_id(key_id, cosigner_id, is_cloud):
    if is_cloud:
        ...
        # print("key_id_first_dword: ", binascii.hexlify(key_id_first_dword).decode())
    ...
    # print(
    #     "player_id (is cloud? " + ("yes" if is_cloud else "no") + "): " + str(player_id)
    # )
...
def recover_autogenerated_mp(mpf_b64, mpf_rsa_key_b64, mpf_rsa_key_pass):
    """
    Recovers auto-generated passphrase
    """
    mobile_passphrase_file_json = json.loads(base64.b64decode(mpf_b64))
    key_pem = base64.b64decode(mpf_rsa_key_b64)

    try:
        key = RSA.importKey(key_pem, mpf_rsa_key_pass)
    except ValueError as exc:
        raise RecoveryErrorRSAKeyImport() from exc
    pprint(key)
    cipher = PKCS1_OAEP.new(key, SHA256)
    try:
        print("Encrypted:", mobile_passphrase_file_json["encryptedKey"])
        return cipher.decrypt(bytes.fromhex(mobile_passphrase_file_json["encryptedKey"]))
    except ValueError as exc:
        print(exc)
        traceback.print_tb(exc.__traceback__)
        raise RecoveryErrorMPFRSADecrypt() from exc
