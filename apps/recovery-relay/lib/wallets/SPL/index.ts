...
import { unsupportedSplAssets } from './unsupported_solana_assets';
...
export class SPL extends BaseSolana implements ConnectedWallet {
  ...
  public async getBalance(): Promise<number> {
    try {
      ...
      this.relayLogger.error('Error in getBalance()', err);
      throw err;
    }
  }
  public async prepare(toAddress: string): Promise<AccountData> {
    try {
      ...
      this.mint = allSplAssets.find((asset) => asset.legacyId === this.assetId)?.onchain.address;
      if (!this.mint) throw new Error('mint not set');
      const mintPubkey = new web3.PublicKey(this.mint);
      ...
      if (!this.decimals) {
        const mintInfo = await this.solConnection!.getParsedAccountInfo(mintPubkey);
        if (!mintInfo.value?.data || !('parsed' in mintInfo.value.data)) {
          throw new Error('Invalid mint account');
        }
        this.decimals = mintInfo.value.data.parsed.info.decimals;
      }
      ...
      try {
        ...
        if (err instanceof TokenAccountNotFoundError) {
          ...
          this.utilityLogger.warn('SPL Error: ', err);
          throw err;
        }
      }
      ...
      this.relayLogger.error('Error in prepare()', err);
      throw err;
    }
  }
