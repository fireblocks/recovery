...
export class DOGE extends BaseDOGE implements ConnectedWallet {
  ...
  private static _satsToBtc(sats: number) {
    return sats / DOGE.satsPerBtc;
  }

  private static _btcToSats(btc: number) {
    return btc * DOGE.satsPerBtc;
  }

  public async prepare(): Promise<AccountData> {
    const utxos = await this._getAddressUTXOs();
    const balance = DOGE._satsToBtc(utxos.map((utxo) => utxo.value).reduce((p, c) => p + c));
  ...
  public async generateTx(
    ...
    const satAmount = DOGE._btcToSats(amount);
    ...
    const balance = DOGE._btcToSats(await this.getBalance());
  ...
  public async broadcastTx(
    ...
    try {
      ...
      this.relayLogger.debug(`DOGE: Tx broadcasted: ${JSON.stringify(txHash, null, 2)}`);
      return txHash;
    } catch (e) {
      this.relayLogger.error(`DOGE: Error broadcasting tx: ${(e as Error).message}`);
      throw e;
    }
  }
