import { Kusama as BaseKSM } from '@fireblocks/wallet-derivation';
...
export class Kusama extends BaseKSM implements SigningWallet {
  public async generateTx({ to, extraParams, nonce }: GenerateTxInput): Promise<TxPayload> {
    ...
    const chainName = this.isTestnet ? 'Westend' : 'Kusama';
    const specName = this.isTestnet ? 'westend' : 'kusama';
    const metadataRpc = mdRpc.kusamaRpc as `0x${string}`;
    ...
    this.utilityLogger.logSigningTx('Kusama', unsigned);
